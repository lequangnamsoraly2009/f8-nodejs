function Validator(options) {
    ...
    function validate(inputElement, x) {
        ...
        if (errorMessage) {
            ...
            getParent(inputElement, options.formInputGroup).classList.add(
                'invalid',
            );
        } else {
            errorElement.innerText = '';
            getParent(inputElement, options.formInputGroup).classList.remove(
                'invalid',
            );
        }
        return !errorMessage;
    ...
    return {
        ...
        test: (value) =>
            value.length >= min
                ? undefined
                : value.length > 0
                ? `Minimum password length is ${min} and includes number`
                : message,
    };
};
